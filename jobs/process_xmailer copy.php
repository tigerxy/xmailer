...
class ProcessXmailer extends AbstractJob
{
    ...
    public function run()
    {
        ...
        if (Config::get('xmailer.addpagename')) {
            $this->listname = '[' . Config::get('concrete.site') . ']';
        }



        $config = Config::get('xmailer.imap');
        $mailbox = new StorageImap(array(
            'host'     => $config['host'],
            'user'     => $config['user'],
    ...
    {
        return $this->listname . ' ' . str_replace($this->listname . ' ', '', $subject);
    }
}